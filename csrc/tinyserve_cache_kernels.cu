...
void tinyserve_fragmentation_analysis(
    ...
    // Allocate device memory for block_ref_counts (simplified - using block_table as reference)
    // In real implementation, this would come from vLLM's block manager
    ...
    cudaMalloc(&d_block_access_patterns, block_table.size(0) * table.max_blocks_per_seq * sizeof(int));
    cudaMalloc(&d_block_priority, total_blocks * sizeof(int));
    cudaMalloc(&d_block_weights, total_blocks * sizeof(float));
    cudaMemset(d_block_access_patterns, 0, block_table.size(0) * table.max_blocks_per_seq * sizeof(int));
    ...
    tinyserve_launch_fragmentation_analysis(
        &table,
        fragmentation_scores.data_ptr<int>(),
        free_block_runs.data_ptr<int>(),
        free_block_run_lengths.data_ptr<int>(),
        num_free_runs.data_ptr<int>(),
        total_blocks,
        stream
    );
...
void tinyserve_fragmentation_aware_allocation(
    ...
    int h_num_free_runs;
    cudaMemcpy(&h_num_free_runs, num_free_runs.data_ptr<int>(), sizeof(int), cudaMemcpyDeviceToHost);
    
    // Launch fragmentation-aware allocation
    tinyserve_launch_fragmentation_aware_allocation(
        &table,
        seq_ids.data_ptr<int>(),
        logical_block_ids.data_ptr<int>(),
        num_blocks_needed.data_ptr<int>(),
        allocated_blocks.data_ptr<int>(),
        fragmentation_scores.data_ptr<int>(),
        free_block_runs.data_ptr<int>(),
        free_block_run_lengths.data_ptr<int>(),
        seq_ids.size(0),
...
void tinyserve_defragmentation(
    ...
    cudaMemset(d_block_ref_counts, 0, table.total_physical_blocks * sizeof(int));
    cudaMemset(d_block_access_patterns, 0, block_table.size(0) * table.max_blocks_per_seq * sizeof(int));
    cudaMemset(d_block_priority, 0, table.total_physical_blocks * sizeof(int));
    cudaMemset(d_block_weights, 0.0f, table.total_physical_blocks * sizeof(float));
    ...
    tinyserve_launch_defragmentation(
        reinterpret_cast<half*>(key_cache.data_ptr()),
        &table,
        block_mapping.data_ptr<int>(),
        blocks_to_move.data_ptr<int>(),
        blocks_to_move.size(0),
        block_size,
        head_dim,
        stream
    );
    ...
    tinyserve_launch_defragmentation(
        reinterpret_cast<half*>(value_cache.data_ptr()),
        &table,
        block_mapping.data_ptr<int>(),
        blocks_to_move.data_ptr<int>(),
        blocks_to_move.size(0),
...
void tinyserve_continuous_block_allocation(
    ...
    torch::Tensor& allocated_block_ranges, // [num_allocations, 2] output [start, end]
    int total_blocks,
    int block_size
) {
    TORCH_CHECK(block_table.device().is_cuda(), "block_table must be on CUDA");
    ...
    tinyserve_launch_continuous_block_allocation(
        ...
        allocated_block_ranges.data_ptr<int>(),
        seq_ids.size(0),
        stream
    );
